...
export default function LeasesPage() {
  // 1. DATA STATES - Now includes pagination meta
  const [data, setData] = useState<any>({ results: [], count: 0, total_pages: 1, current_page: 1, next: null, previous: null });
  ...
  // 🔥 ADDED 'page' to filters
  const [filters, setFilters] = useState({ status: "", search: "", page: 1 });

  // 3. LOAD DATA - Updated to handle pagination envelope
  const loadData = async () => {
    setLoading(true);
    try {
      // Assuming LeaseService.list accepts { status, search, page }
  ...
  // 🔥 Trigger load on page change, search change, or status change
  ...
  // Use data.results for calculations
  const stats = useMemo(() => ({
    ...
    total: data.count // Using the total count from API
  }), [data]);
            ...
            <div className="d-flex gap-2">
              <div className="input-group input-group-sm" style={{ maxWidth: '250px' }}>
                ...
                <input
                   ...
                   value={filters.search} onChange={e => setFilters({...filters, search: e.target.value, page: 1})} // 🔥 Reset page on search
                />
              </div>
              <select
                 className="form-select form-select-sm bg-light border-0 w-auto"
                 value={filters.status} onChange={e => setFilters({...filters, status: e.target.value, page: 1})} // 🔥 Reset page on filter
              >
              ...
              <table className="table table-hover align-middle mb-0">
                <thead className="bg-light">
                  <tr className="text-muted small text-uppercase fw-bold">
                    ...
                    <th colSpan={6} className="d-md-none ps-3 py-3 text-primary">
                        <i className="bi bi-list-task me-2"></i>Active Agreements
                    </th>
                  </tr>
                </thead>
                <tbody>
