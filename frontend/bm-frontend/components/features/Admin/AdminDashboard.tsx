...
export default function AdminDashboard() {
    ...
    const [isOverdueChecking, setIsOverdueChecking] = useState(false);
    ...
    const handleManualReminder = async () => {
        if (!window.confirm("Send friendly SMS/Email reminders to all renters with upcoming dues?")) return;
        setIsReminding(true);
        try {
    ...
    const handleOverdueNotice = async () => {
        if (!window.confirm("⚠️ Send URGENT overdue notices to renters with dues older than 30 days?")) return;
                ...
                <div className="card border-0 shadow-sm rounded-4 h-100 animate__animated animate__fadeIn">
                    <div className="card-body d-flex flex-column flex-md-row align-items-center gap-2 gap-md-3 p-3">
                        ...
                        <div className="text-center text-md-start">
                            <h6 className="text-muted mb-0 fw-bold text-uppercase"
                                style={{fontSize: '0.6rem', letterSpacing: '0.5px'}}>{title}</h6>
    ...
    return (
        <div className="container-fluid px-2 px-md-3">
            ...
            <div className="row g-2 g-md-4">
                <StatCard title="Revenue" value={summary.total_income} icon="bi-cash-stack" color="success"
                          isCurrency={true}/>
                <StatCard title="Dues" value={summary.total_due} icon="bi-exclamation-triangle" color="danger"
                          isCurrency={true}/>
                <StatCard title="Renters" value={summary.active_renters} icon="bi-people" color="primary"/>
                <StatCard title="Occupancy" value={`${occupancy.occupancy_percent}%`} icon="bi-building-check"
                          color="info"/>
            </div>
            ...
            <div className="row mt-2 mt-md-4">
                ...
                <div className="col-lg-8 mb-4">
                    <div className="card border-0 shadow-sm rounded-4 bg-white h-100">
                        <div
                            className="card-header bg-white border-0 pt-4 px-3 px-md-4 d-flex justify-content-between align-items-center">
                                        ...
                                        <tr key={payment.id} style={{cursor: 'pointer'}}>
                                            <td className="ps-3 ps-md-4">
                                                ...
                                                <div className="text-muted"
                                                     style={{fontSize: '0.7rem'}}>{payment.unit_name || "N/A"}</div>
                                            </td>
                                            <td className="d-none d-sm-table-cell">
                                                <span
                                                    className="badge bg-light text-dark border x-small">{payment.method}</span>
                                            </td>
                                            <td>
                                                <div
                                                    className="fw-bold text-success small">৳{Number(payment.amount).toLocaleString()}</div>
                                                <div
                                                    className="d-block d-md-none text-muted x-small">{payment.payment_date}</div>
                                            </td>
                                        </tr>
                                    ))}
                                    </tbody>
                                ...
                                {topDues.slice(0, 5).map((renter: any) => (
                                    <div key={renter.renter_id}
                                        ...
                                        <div className="text-end">
                                            <div
                                                className="text-danger fw-bold small">৳{Number(renter.total_due).toLocaleString()}</div>
                                            <button className="btn btn-link p-0 x-small text-decoration-none"
                                                    onClick={() => router.push(`/admin-dashboard/renters/${renter.renter_id}`)}>View
                                            </button>
                                        </div>
                                    </div>
                                ))}
