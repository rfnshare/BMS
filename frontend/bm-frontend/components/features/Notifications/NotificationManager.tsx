...
export default function NotificationManager() {
  ...
  // 🚀 SQA FIX: Separate 'uiSearch' from 'activeFilters' to prevent constant re-renders
  ...
  useEffect(() => {
    ...
    debounceTimer.current = setTimeout(() => {
      // Only update the active filter if the search string actually changed
      setFilters(prev => ({ ...prev, search: uiSearch, page: 1 }));
    }, 500); // 500ms delay to let the user finish typing
        ...
        <Container className="px-0" style={{ maxWidth: '800px' }}>
          ...
          <div className="d-flex gap-2">
            <input
              ...
              value={uiSearch} // Uses UI state, not filter state directly
