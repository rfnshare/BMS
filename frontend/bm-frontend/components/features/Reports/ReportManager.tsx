    ...
    return (
        <div className="bg-light min-vh-100 p-2 p-md-4">
            ...
            <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
                ...
                {/* 🚀 FIXED TABS: Removed flex-fill on desktop, added px-md-4 for better width */}
                <div
                    className="btn-group w-100 w-md-auto bg-white rounded-pill p-1 shadow-sm overflow-hidden"
                    style={{border: '1px solid #eee'}}
                >
                    {['financial', 'occupancy', 'renters'].map(tab => (
                        <button
                            key={tab}
                            className={`btn btn-sm rounded-pill fw-bold transition-all ${
                                activeTab === tab
                                    ? 'btn-dark text-white shadow-sm'
                                    : 'text-muted border-0 bg-transparent'
                            } ${
                                /* flex-fill only on mobile (< 768px), auto-width with padding on desktop */
                                'flex-fill flex-md-grow-0 px-md-4 py-md-2'
                            }`}
                    ...
                    <Row className="g-3">
                        ...
                        <Col xs={12} md={6}>
                            <Card className="border-0 shadow-sm rounded-4 overflow-hidden bg-white">
                                <div
                                    className="card-header bg-danger text-white p-3 fw-bold border-0 small text-uppercase">Critical
                                    Dues (Immediate Attention)
                                </div>
                        ...
                        <Col xs={12} md={6}>
                            <Card className="border-0 shadow-sm rounded-4 overflow-hidden bg-white">
                                <div
                                    className="card-header bg-success text-white p-3 fw-bold border-0 small text-uppercase">Payment
                                    Performance
                                </div>
                                <div className="table-responsive" style={{maxHeight: '400px'}}>
                                    <table className="table table-hover align-middle mb-0">
                                        ...
                                        <tr>
                                            <th>Renter</th>
                                            <th className="text-end pe-3">Paid/Due</th>
                                        </tr>
                                        </thead>
                                        <tbody>
                                        {data.renters.collection.map((ren: any) => (
                                            <tr key={ren.renter_id}>
                                                ...
                                                <td className="text-end pe-3 small">
                                                    <div
                                                        className="text-success fw-bold">৳{Number(ren.total_paid).toLocaleString()}</div>
                                                    <div
                                                        className={Number(ren.total_due) < 0 ? "text-primary x-small fw-bold" : "text-danger x-small fw-bold"}>
