...
// 1. Added Toast and ToastContainer
import { Modal, Button, Form, Row, Col, Badge, Toast, ToastContainer } from "react-bootstrap";
...
export default function PermissionModal({ rule, onClose, onSuccess }: any) {
  ...
  const triggerNotify = (msg: string, variant: string = "success") => {
    setToastConfig({ message: msg, variant });
    setShowToast(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.app_label || !formData.model_name) {
      return triggerNotify("Please select an App and Resource", "danger");
    }
    ...
    try {
      ...
      triggerNotify(rule ? "Policy updated!" : "New policy created!");

      // Delay allows user to see the success toast before modal unmounts
      setTimeout(() => {
        onSuccess();
      }, 1200);
    } catch (err) {
      triggerNotify("This policy already exists or there was a server error", "danger");
    }
  };

  // 4. Close Wrapper
  const handleClose = () => {
    triggerNotify("Action cancelled", "secondary");
    setTimeout(() => {
      onClose();
    }, 800);
  };
  ...
  return (
    <>
      {/* 5. Toast UI - Positioned at top-center with high z-index to overlay modal */}
      ...
      {/* 6. Updated onHide to use handleClose */}
      <Modal show onHide={handleClose} centered size="lg" fullscreen="sm-down">
        ...
        <Form onSubmit={handleSubmit} className="d-flex flex-column h-100">
          ...
          <Modal.Footer className="p-3 bg-light border-top">
            {/* Added triggerNotify to the Cancel button via handleClose */}
            <Button variant="outline-secondary" className="border-0 px-4" onClick={handleClose}>Cancel</Button>
