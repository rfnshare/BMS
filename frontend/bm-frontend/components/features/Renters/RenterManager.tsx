...
export default function RenterManager() {
    ...
    const [activeModal, setActiveModal] = useState<{ type: 'edit' | 'create' | 'docs' | null, data: any }>({
        type: null,
        data: null
    });
        ...
        <div className="animate__animated animate__fadeIn">
                ...
                <div className="card-body p-3 p-md-4">
                    <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
                        <div>
                            ...
                            <p className="text-muted x-small mb-0 text-uppercase fw-bold ls-1 opacity-75">Tenant
                                Profiles & Legal Identity</p>
                        </div>
                        <Button
            ...
            <Row className="g-2 g-md-3 mb-4">
                {[
                    ...
                    <Col key={i} xs={4} md={4}>
                        <div
                            className={`card border-0 shadow-sm rounded-4 p-2 p-md-3 border-start border-4 border-${item.color} bg-white h-100`}>
                            <small className="text-muted fw-bold text-uppercase ls-1 d-none d-md-block"
                                   style={{fontSize: '0.6rem'}}>{item.label}</small>
                            ...
                            <small className="text-muted fw-bold text-uppercase ls-1 d-md-none"
                                   style={{fontSize: '0.5rem'}}>{item.label}</small>
                        </div>
                    </Col>
                ))}
            </Row>
            ...
            {loading && renters.length === 0 ? (
                ...
                <>
                    {/* --- 4. OPTIMIZED MOBILE CARD VIEW --- */}
                    <div className="d-block d-md-none vstack gap-3 mb-4">
                        {renters.map((r) => (
                            <div key={r.id}
                                 className="card border-0 shadow-sm rounded-4 p-3 bg-white border-start border-4 border-primary">
                                ...
                                <div className="d-flex justify-content-between align-items-start mb-3">
                                    <div className="d-flex align-items-center gap-2">
                                        {r.profile_pic ? (
                                            ...
                                            <div
                                                className="rounded-circle bg-primary bg-opacity-10 text-primary d-flex align-items-center justify-content-center fw-bold border"
                                                style={{width: "40px", height: "40px"}}>
                                                {r.full_name.substring(0, 1)}
                                            </div>
                                        )}
                                        <div>
                                            <div className="fw-bold text-dark lh-1">{r.full_name}</div>
                                            <small className="text-muted x-small fw-bold ls-1">UID: #{r.id}</small>
                                        </div>
                                    </div>
                                ...
                                <div className="bg-light rounded-3 p-2 mb-3 d-flex justify-content-around">
                                    <div className="small fw-bold text-dark">
                                        <i className="bi bi-telephone text-primary me-2"></i>{r.phone_number}
                                    </div>
                                    <div className="vr text-muted opacity-25"></div>
                                    <div className="small fw-bold text-truncate ms-2" style={{maxWidth: '150px'}}>
                                        <i className="bi bi-envelope text-info me-2"></i>{r.email || 'N/A'}
                                    </div>
                                </div>

                                {/* Bottom Row: Full-Width Management Actions */}
                                <div className="d-flex gap-2">
                                    <button
                                        className="btn btn-sm btn-outline-primary flex-grow-1 rounded-pill py-2"
                                        onClick={() => router.push(`/admin-dashboard/renters/${r.id}`)}
                                    >
                                        <i className="bi bi-speedometer2"></i>
                                    </button>
                                    <button
                                        className="btn btn-sm btn-outline-info flex-grow-1 rounded-pill py-2"
                                        onClick={() => setActiveModal({type: 'docs', data: r})}
                                    >
                                        <i className="bi bi-shield-lock"></i>
                                    </button>
                                    <button
                                        className="btn btn-sm btn-outline-warning flex-grow-1 rounded-pill py-2"
                                        onClick={() => setActiveModal({type: 'edit', data: r})}
                                    >
                                        <i className="bi bi-pencil-square"></i>
                                    </button>
                                    <button
                                        className="btn btn-sm btn-outline-danger flex-grow-1 rounded-pill py-2"
                                        onClick={async () => {
                                            if (confirm("Permanent purge this resident record?")) {
                        ...
                        <table className="table table-hover align-middle mb-0">
                            ...
                            {renters.map((r) => (
                                <tr key={r.id}>
                                        ...
                                        <div className="d-flex align-items-center gap-3">
                                            <Link href={`/admin-dashboard/renters/${r.id}`}>
                                                {r.profile_pic ? (
                                                    ...
                                                    <div
                                                        className="rounded-circle bg-primary bg-opacity-10 text-primary d-flex align-items-center justify-content-center fw-bold border"
                                                        style={{width: "42px", height: "42px", fontSize: "0.8rem"}}>
                                    ...
                                    <td>
                                        <div
                                            className="small fw-semibold text-primary">{r.email || 'No email registered'}</div>
                                    </td>
                                    ...
                                    <td className="pe-4 text-end">
                                        <div
                                            className="btn-group shadow-sm border rounded-pill overflow-hidden bg-white">
                                            <button className="btn btn-sm btn-white px-3 border-end"
                                                    onClick={() => router.push(`/admin-dashboard/renters/${r.id}`)}
                                                    title="Dashboard"><i
                                                className="bi bi-speedometer2 text-primary"></i></button>
                                            <button className="btn btn-sm btn-white px-3 border-end"
                                                    onClick={() => setActiveModal({type: 'docs', data: r})}
                                                    title="Vault"><i className="bi bi-shield-lock text-info"></i>
                                            </button>
                                            <button className="btn btn-sm btn-white px-3 border-end"
                                                    onClick={() => setActiveModal({type: 'edit', data: r})}
                                                    title="Edit Profile"><i
                                                className="bi bi-pencil-square text-warning"></i></button>
                                            <button className="btn btn-sm btn-white px-3 text-danger"
                                                    onClick={async () => {
                                                        if (confirm("Permanent purge this resident record?")) {
                                                            await RenterService.destroy(r.id);
                                                            success("Record purged.");
                                                    ...
                                                    }} title="Purge"><i className="bi bi-trash3"></i></button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
            ...
            {pagination.totalPages > 1 && (
                <div
                    className="d-flex justify-content-between align-items-center bg-white p-3 rounded-4 shadow-sm border mb-5">
                    <div className="small text-muted fw-bold ls-1 text-uppercase"
                         style={{fontSize: '0.6rem'}}>Page {filters.page} of {pagination.totalPages}</div>
                    <div className="d-flex gap-2">
                        <button className="btn btn-outline-secondary btn-sm rounded-pill px-4 fw-bold ls-1"
            ...
            {(activeModal.type === 'create' || activeModal.type === 'edit') && (
                <RenterModal renter={activeModal.data} onClose={() => setActiveModal({type: null, data: null})}
                             onSaved={load}/>
            )}
            {activeModal.type === 'docs' && (
                <RenterDocumentsModal renter={activeModal.data}
