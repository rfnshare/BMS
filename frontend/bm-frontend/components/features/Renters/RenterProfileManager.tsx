        ...
        <div className="col-md-6 col-lg-4 mb-4">
            <div className="d-flex align-items-center gap-2 mb-1">
                ...
                <span className="fw-bold text-uppercase text-muted small"
                      style={{letterSpacing: '0.5px'}}>{label}</span>
            </div>
        ...
        <div className="container-fluid py-4 bg-light min-vh-100">
            ...
            <div className="card border-0 shadow-sm rounded-4 overflow-hidden mb-4">
                <div className="bg-primary p-5"
                     style={{background: 'linear-gradient(45deg, #0d6efd, #0099ff)', height: '120px'}}></div>
                        ...
                        <div className="flex-grow-1 text-center text-md-start pb-2">
                            <div className="d-flex flex-column flex-md-row align-items-center gap-2">
                                ...
                                <div className="d-flex gap-2">
                                    <Badge pill bg={renter.status === "active" ? "success" : "secondary"}
                                           className="px-3 py-2">
            ...
            <div className="row g-4">
                <div className="col-lg-8">
                    ...
                    {lease ? (
                        <div
                            className={`card border-0 shadow-sm rounded-4 mb-4 overflow-hidden border-start border-4 ${lease.status === 'active' ? 'border-success' : 'border-secondary opacity-75'}`}>
                            <div className="card-body p-4">
                                <div className="d-flex justify-content-between align-items-start mb-3">
                                    ...
                                    <button
                                        className={`btn btn-sm rounded-pill px-3 fw-bold ${lease.status === 'active' ? 'btn-outline-success' : 'btn-outline-secondary'}`}
                                ...
                                <div className="row g-3">
                                    <div className="col-md-4">
                                        <small className="text-muted fw-bold text-uppercase x-small d-block">Unit &
                                            Floor</small>
                                    ...
                                    <div className="col-md-4">
                                        ...
                                        <span
                                            className={`fw-bold fs-5 ${Number(lease.current_balance) > 0 ? 'text-danger' : 'text-dark'}`}>
                        ৳{Number(lease.status === 'active' ? lease.rent_amount : lease.current_balance).toLocaleString()}
                    </span>
                                        <div className="small text-muted">Status: <span
                                            className="text-capitalize">{lease.deposit_status}</span></div>
                                    </div>
                                    <div className="col-md-4">
                                        <small className="text-muted fw-bold text-uppercase x-small d-block">
                                            {lease.status === 'active' ? 'Lease Period' : 'Termination Date'}
                                        </small>
          ...
          Staying for: {calculateTenure(lease.start_date)}
                                            ...
                                            {lease.end_date ? (
                                                <div className="small text-muted mt-1">
                                                    <i className="bi bi-arrow-right-short"></i> To: {new Date(lease.end_date).toLocaleDateString('en-GB', {
                                                    day: '2-digit',
                                                    month: 'short',
                                                    year: 'numeric'
                                                })}
                                                ...
                                                <div className="small text-success-emphasis opacity-75 mt-1"
                                                     style={{fontSize: '0.75rem'}}>
                                                    <i className="bi bi-infinity"></i> Ongoing Lease
                                                </div>
                                            ) : null}
                                        </div>
                                    </div>
                        ...
                        <div
                            className="alert alert-info rounded-4 border-0 shadow-sm p-4 d-flex align-items-center mb-4">
                        ...
                        <div className="card-body p-4">
                            ...
                            <div className="row">
                                ...
                                <InfoRow label="Reason for Leaving" value={renter.reason_for_leaving}
                                         icon="chat-left-text"/>
                                <InfoRow label="Stay Duration"
                                         value={`${renter.from_date || ''} to ${renter.to_date || ''}`}
                                         icon="clock-history"/>
                            </div>
                        </div>
                    </div>
                ...
                <div className="col-lg-4">
                    <div
                        className="card border-0 shadow-sm rounded-4 mb-4 bg-danger-subtle border-start border-danger border-4">
                        <div className="card-body p-4">
                                ...
                                <div className="d-flex align-items-center p-3 border rounded-3 mb-3 bg-light">
                                    ...
                                    <div className="flex-grow-1">
                                        ...
                                        <a href={renter.nid_scan} target="_blank" rel="noreferrer"
                                           className="text-decoration-none small">View Identification</a>
                                    </div>
                                </div>
                            )}
                            ...
                            {renter.documents?.map((doc: any) => (
                                <div key={doc.id} className="d-flex align-items-center p-3 border rounded-3 mb-2">
                                    ...
                                    <div className="flex-grow-1 overflow-hidden">
                                        <div
                                            className="fw-bold small text-capitalize text-truncate">{doc.doc_type}</div>
                                        <a href={doc.file} target="_blank" rel="noreferrer"
                                           className="text-decoration-none small">Download File</a>
                                    </div>
                                </div>
                            ))}

                            <div className="mt-4 pt-3 border-top">
                                <button className="btn btn-outline-secondary w-100 rounded-pill btn-sm fw-bold"
            ...
            {showEditModal &&
                <RenterModal renter={renter} onClose={() => setShowEditModal(false)} onSaved={() => loadAllData()}/>}
