        ...
        <div className="animate__animated animate__fadeIn">
                ...
                <div className="card-body p-3 p-md-4">
                    <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
                        <div>
                            ...
                            <p className="text-muted x-small mb-0 text-uppercase fw-bold ls-1">Ticketing & Issue
                                Resolution</p>
                        </div>
                        <button
            ...
            <div className="card border-0 shadow-sm rounded-4 overflow-hidden bg-white d-none d-md-block">
                <table className="table table-hover align-middle mb-0">
                    ...
                    {loading ? (
                        <tr>
                            <td colSpan={5} className="text-center py-5"><Spinner animation="border" variant="primary"
                                                                                  size="sm"/></td>
                        </tr>
                            ...
                            <td className="pe-4 text-end">
                                <div className="btn-group shadow-sm border rounded-3 overflow-hidden bg-white">
                                    <button className="btn btn-sm btn-white border-end"
                                            onClick={() => setActiveModal({type: 'edit', data: c})}><i
                                        className="bi bi-pencil-square text-primary"></i></button>
                                    <button className="btn btn-sm btn-white text-danger"
                                            onClick={() => handleDelete(c.id)}><i className="bi bi-trash"></i></button>
                                </div>
                            </td>
                        </tr>
                    ))}
            ...
            <div className="d-block d-md-none bg-light-subtle">
                {loading ? (
                    ...
                    data.results.map((c: any) => (
                        <div key={c.id} className="p-3 border-bottom bg-white mb-2 shadow-sm">
                            ...
                            <div className="d-flex justify-content-between align-items-center mb-2">
                                ...
                                <div className="btn-group">
                                    <button
                                        className="btn btn-sm btn-light border py-1 px-3"
                                        onClick={() => setActiveModal({type: 'edit', data: c})}
                                    >
                                        <i className="bi bi-pencil-square text-primary"></i>
                                    </button>
                                    <button
                                        className="btn btn-sm btn-light border text-danger py-1 px-3"
                            ...
                            <div className="d-flex justify-content-between align-items-end mt-2">
                                <div className="x-small text-muted">
                                    <div className="mb-1"><i
                                        className="bi bi-house-door me-1"></i>{c.unit_name || 'General'}</div>
                                    <div><i className="bi bi-person me-1"></i>{c.renter_name || 'Internal'}</div>
                                </div>
                                <div className="text-end">
                                    <Badge pill className={`border x-small d-block mb-1 ${getStatusBadge(c.status)}`}>
                                        {c.status?.replace('_', ' ').toUpperCase()}
                                    </Badge>
                                    <span className="text-muted x-small" style={{fontSize: '0.65rem'}}>
              {new Date(c.created_at).toLocaleDateString()}
                    ...
                    /* ✅ Fallback: Shows if the list is actually empty */
                    <div className="text-center py-5 bg-white">
                        <i className="bi bi-clipboard-check display-1 text-light"></i>
                        <p className="text-muted mt-2">No maintenance tickets found.</p>
                    </div>
                )}
            </div>
